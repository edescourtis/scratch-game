...
Problem statement: You need to build a scratch game that generates a matrix (for example 3 × 3) from symbols (based on per-cell probabilities). Based on the winning combinations, the user will either win or lose.
...
Bonus symbols are described in the table below:
...
| symbol.{X}.impact                                    | [only for bonuses] fixed values: multiply_reward (which multiplies the final reward by *symbol.{X}.reward_multiplier*), extra_bonus (adds *symbol.{X}.extra* to the final reward), miss (no effect)                     |
